...
# LLM and Pipeline imports are intentionally REMOVED to save memory.
from collections import defaultdict

# --- LAZY LOADING IMPLEMENTATION ---
_INTERACTIONS_DF = None
_MEDICINES_DF_STATIC = None
# MODIFIED: This cache will now store Retrievers, which use less memory.
_RETRIEVER_CACHE = {} 
...
def get_medicine_info(query: str):
    """
    MODIFIED: This function now gets a 'retriever' instead of a full 'rag_chain'.
    It directly finds the most relevant piece of information.
    """
    retriever = create_retriever(search_k=1)
    if retriever is None: return "Error: Could not create the information retrieval system."
    
    # Use the retriever to get the most relevant document directly.
...
def intelligent_symptom_analyser(query: str):
    # This function is unchanged as it does not use the heavy AI models.
...
def create_retriever(search_k=1):
    """
    MODIFIED: This function is renamed from 'create_rag_chain' to 'create_retriever'
    and it no longer loads the heavy 'distilgpt2' LLM.
    """
    ...
    # REMOVED: The llm = HuggingFacePipeline(...) line is gone.
    # REMOVED: The RetrievalQA chain is gone.

    # We now create and cache the retriever directly.
...
def get_ai_response(query: str):
    # This routing logic is unchanged.
    _load_static_data() 
