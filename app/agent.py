...
# --- 1. LOAD STATIC DATA SOURCES ---
print("Loading static data sources...")
try:
    interactions_df = pd.read_csv("data/interactions.csv", encoding='latin1')
    medicines_df_static = pd.read_csv("data/medicines.csv", encoding='latin1')
    print("Static data loaded successfully.")
except FileNotFoundError as e:
    print(f"FATAL ERROR: Could not load a required CSV file. {e}")
    interactions_df = pd.DataFrame()
    medicines_df_static = pd.DataFrame()
...
# <<< FINAL, CONTEXT-AWARE VERSION >>> Tool 2: Intelligent Symptom Analyser
def intelligent_symptom_analyser(query: str):
    print("--- Running Intelligent Symptom Analyser (Context-Aware) ---")
    
    medicines_df = medicines_df_static.copy()
    medicines_df.fillna('N/A', inplace=True)

    # Step 1: Create a more detailed knowledge graph from the CSV.
    use_case_map = defaultdict(list)
    for index, row in medicines_df.iterrows():
        # Split use cases like "Joint Pain, Sprain" into individual terms
    ...
    # Step 2: More intelligently extract symptoms and context from the query.
    query_lower = query.lower().replace('nouse', 'nose').replace('pain', ' pain ') # Standardize
    
    # Define symptom categories and their keywords
    symptom_categories = {
    ...
    # Check for specific pain types first
    ...
    if not detected_symptoms:
        return "Please describe your symptoms more clearly (e.g., 'I have a headache and a cough') for an analysis."

    # Step 3: Use a smarter rule-based system to suggest conditions.
    possible_conditions = []
    ...
    # Step 4: Match detected keywords to the knowledge graph to find medicines.
    suggested_medicines = {}
    ...
    for keyword in all_detected_keywords:
        # Clean the keyword (e.g., "knee pain" -> "pain") for broader matching
        search_term = keyword.split(' ')[-1] 
        if search_term in use_case_map:
            med_name = use_case_map[search_term][0] # Get the first match
    ...
    # Step 5: Build the final, structured response.
...
def check_drug_interactions(query: str):
    mentioned_drugs = [name for name in medicines_df_static['Medicine_Name'].unique() if re.search(r'\b' + re.escape(name) + r'\b', query, re.IGNORECASE)]
    ...
    interaction = interactions_df[
        ((interactions_df['Medicine_A'].str.lower() == drug1.lower()) & (interactions_df['Medicine_B'].str.lower() == drug2.lower())) |
        ((interactions_df['Medicine_A'].str.lower() == drug2.lower()) & (interactions_df['Medicine_B'].str.lower() == drug1.lower()))
    ]
...
def get_ai_response(query: str):
    ...
    mentioned_drugs = set(name for name in medicines_df_static['Medicine_Name'].unique() if re.search(r'\b' + re.escape(name) + r'\b', query, re.IGNORECASE))
...
print("AI Agent is ready with Final Intelligent Symptom Analyser.")